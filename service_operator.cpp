...
DEFINE_bool(hook, true, "hook event type to event function, when set to false, unhook the event type");
DEFINE_string(events, "0,1,2,3,4",
        "a string of event types seperated by comma to hook/unhook, only 0,1,2,3,4 are allowed at present, 0: registeruser, 1: warble, 2: follow, 3: read, 4: profile"
        );
